...
use crate::nodes::service::{
    random_alias, CredentialRetrieverCreators, CredentialScope,
    NodeManagerCredentialRetrieverOptions, NodeManagerTrustOptions,
};
use crate::session::MedicHandle;
use crate::{CliState, DefaultAddress, EnrollmentStatus};
...
impl NodeManager {
    ...
    pub async fn create(
        ...
        > = match trust_options.project_member_credential_retriever_options {
            ...
            NodeManagerCredentialRetrieverOptions::CacheOnly { issuer, project_id } => {
                Some(Arc::new(CachedCredentialRetrieverCreator::new(
                    issuer.clone(),
                    CredentialScope::ProjectMember { project_id }.to_string(),
                    secure_channels.identities().cached_credentials_repository(),
                )))
            }
            NodeManagerCredentialRetrieverOptions::Remote { info, project_id } => {
        ...
        > = match trust_options.project_admin_credential_retriever_options {
            ...
            NodeManagerCredentialRetrieverOptions::CacheOnly { issuer, project_id } => {
                Some(Arc::new(CachedCredentialRetrieverCreator::new(
                    issuer.clone(),
                    CredentialScope::ProjectAdmin { project_id }.to_string(),
                    secure_channels.identities().cached_credentials_repository(),
                )))
            }
            NodeManagerCredentialRetrieverOptions::Remote { info, project_id } => {
